<?php
class MateusNeves_SalesRule_Model_Validator extends Mage_SalesRule_Model_Validator
{
    /**
    ...
    public function process(Mage_Sales_Model_Quote_Item_Abstract $item)
    {
        ...
        $SAppliedRuleIds = Mage::getSingleton('core/session')->getSAppliedRuleIds();
        if (empty($SAppliedRuleIds)) {
        	Mage::getSingleton('core/session')->setSAppliedRuleIds($appliedRuleIds);
        }

        if ($SAppliedRuleIds != $appliedRuleIds) {
        	foreach ($appliedRuleIds as $id) {
				if (!in_array($id, $SAppliedRuleIds)) {
					$rulez = Mage::getModel('salesrule/rule')->load($id);

					Mage::getSingleton('core/session')->addNotice('Desconto: "'.$rulez->getName().'", aplicado.');
				}
			}
        }
